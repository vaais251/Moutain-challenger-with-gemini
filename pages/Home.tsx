...
import AIAdventureFinder from '../components/AIAdventureFinder';
...
const Hero = () => (
    <div className="relative h-screen bg-cover bg-center" style={{ backgroundImage: "url('https://picsum.photos/seed/k2-hero/1920/1080')" }}>
        ...
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-center text-white px-4">
            <h1 className="font-heading text-5xl font-extrabold tracking-tight uppercase md:text-7xl lg:text-8xl animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-4 animate-fade-in-up" style={{ animationDelay: '0.6s' }}>
                ...
                <Link
                    to="/ai-planner"
                    className="transform rounded-md border-2 border-brand-accent px-8 py-3 text-lg font-semibold text-brand-accent shadow-lg transition hover:-translate-y-1 hover:bg-brand-accent hover:text-brand-dark"
                >
                    AI Trip Planner
                </Link>
            </div>
...
const ExpeditionCard: React.FC<{ expedition: typeof EXPEDITIONS[0] }> = ({ expedition }) => (
    <div className="group relative overflow-hidden rounded-lg shadow-2xl animate-fade-in-up">
        ...
        <div className="absolute bottom-0 left-0 p-6 text-white">
            <h3 className="text-2xl font-bold font-heading">{expedition.name}</h3>
            ...
            <Link to={`/expeditions/${expedition.id}`} className="mt-6 inline-block rounded-md bg-brand-accent px-5 py-2 text-sm font-bold text-brand-dark transition-opacity duration-300 opacity-0 group-hover:opacity-100">
                View Details
            </Link>
        </div>
    </div>
    ...
    return (
        <div className="py-24 bg-brand-base">
            <div className="container mx-auto px-4">
                <h2 className="font-heading text-4xl font-extrabold text-center uppercase text-white">Why Mountain Challenger?</h2>
                <p className="mt-4 text-center text-lg text-brand-muted max-w-3xl mx-auto">We are more than a tour company; we are your partners in adventure, committed to delivering an experience that is authentic, safe, and unforgettable.</p>
                <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-12">
                    {features.map((feature, index) => (
                         <div key={index} className="text-center p-6 bg-brand-dark rounded-lg shadow-lg animate-fade-in-up" style={{ animationDelay: `${index * 0.2}s` }}>
                            <div className="text-5xl">{feature.icon}</div>
                            <h3 className="mt-4 text-2xl font-bold font-heading text-white">{feature.title}</h3>
...
const UpcomingDepartures = () => {
    ...
    return (
        <div className="py-24 bg-brand-dark">
            <div className="container mx-auto px-4">
                <h2 className="font-heading text-4xl font-extrabold text-center uppercase text-white">Upcoming Departures</h2>
                <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-8">
                    {departures.map((departure, index) => (
                        <div key={index} className="bg-brand-base rounded-lg shadow-xl overflow-hidden animate-fade-in-up" style={{ animationDelay: `${index * 0.2}s` }}>
                            ...
                            <div className="p-6">
                                ...
                                <h3 className="font-heading text-xl font-bold text-white mt-2">{departure.expedition.name}</h3>
                                <div className="mt-4 flex justify-between items-center text-sm text-brand-muted">
                                    <span>{departure.expedition.duration} Days</span>
                                    <span>{departure.expedition.difficulty}</span>
                                </div>
                                <Link to={`/expeditions/${departure.expedition.id}`} className="mt-6 inline-block w-full text-center rounded-md bg-brand-accent px-5 py-2 text-sm font-bold text-brand-dark transition hover:bg-lime-400">
                                    Join This Trip
                                </Link>
                            </div>
                        </div>
...
const LatestBlogPosts = () => (
    <div className="py-24 bg-brand-base">
        <div className="container mx-auto px-4">
            <h2 className="font-heading text-4xl font-extrabold text-center uppercase text-white">From Our Blog</h2>
            <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-10 max-w-4xl mx-auto">
                {BLOG_POSTS.slice(0, 2).map((post, index) => (
                    <div key={post.id} className="group bg-brand-dark rounded-lg shadow-lg overflow-hidden animate-fade-in-up" style={{ animationDelay: `${index * 0.2}s` }}>
                        <Link to={`/blog/${post.id}`}>
                            ...
                            <div className="p-6">
                                <h3 className="font-heading text-xl font-bold text-white group-hover:text-brand-accent">{post.title}</h3>
                                <p className="text-sm text-brand-muted mt-2">{post.date} by {post.author}</p>
                                <p className="text-brand-light mt-4">{post.excerpt}</p>
                            </div>
                        </Link>
                    </div>
                ))}
...
const Testimonials = () => (
    <div className="py-24 bg-brand-dark">
        <div className="container mx-auto px-4">
            <h2 className="font-heading text-4xl font-extrabold text-center uppercase text-white">Hear From Our Challengers</h2>
            <div className="mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {TESTIMONIALS.map((testimonial, index) => (
                    <div key={testimonial.id} className="bg-brand-base p-8 rounded-lg shadow-xl animate-fade-in-up" style={{ animationDelay: `${index * 0.2}s` }}>
                        <p className="text-brand-light italic">"{testimonial.quote}"</p>
    ...
    return (
        <div>
            ...
            <section className="py-24 bg-brand-dark">
                <div className="container mx-auto px-4">
                    <h2 className="font-heading text-4xl font-extrabold text-center uppercase text-white">Featured Expeditions</h2>
